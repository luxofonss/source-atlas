        ...
        try:
            for chunk in chunks:
                ...
                for field in chunk.fields:
                    clean_type = self._clean_type_name(field.full_type)
                    if clean_type and not self._is_builtin_type(clean_type):
                        graph.add_edge(chunk.full_class_name, clean_type, "field")
                
                # Add dependencies from method parameters, return types, throws, and calls
                for method in chunk.methods:
                    # Parameters
                    for param_name, param_type in method.parameters:  # Extract type from (name, type) tuple
                        clean_type = self._clean_type_name(param_type)
                        if clean_type and not self._is_builtin_type(clean_type):
                            graph.add_edge(chunk.full_class_name, clean_type, "parameter")
                    
                    # Return type
                    clean_type = self._clean_type_name(method.full_return_type)
                    if clean_type and not self._is_builtin_type(clean_type):
                        graph.add_edge(chunk.full_class_name, clean_type, "return")
                    
                    # Throws
                    for throw_type in method.throws:
                        clean_type = self._clean_type_name(throw_type)
                        if clean_type and not self._is_builtin_type(clean_type):
                            graph.add_edge(chunk.full_class_name, clean_type, "throws")
